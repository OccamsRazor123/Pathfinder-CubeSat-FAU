# Updated to save JSON file for daily tracking, ie how much water and amount of LUX delivered.
# Also i corrected temp threshold ranges and added fans to turn on based on RH as well
# Still need to troubleshoot humming pump
...
# Humidity Thresholds
RH_THRESHOLD_HIGH = 60.0 # Humidity % threshold to turn on fans
RH_P_BAND = 10.0
# Humidity % over threshold to reach 100% fan power. Meaning fans at max at 70% RH.
...
def save_mission_state(filename, day_start, lux_sec, dose_delivered):
    """Saves mission state atomically to prevent corruption during power loss."""
    state = {
    ...
    try:
        with open(temp_file, 'w') as f:
            json.dump(state, f)
        os.replace(temp_file, filename) # Instantly overwrites the old file safely
    except Exception as e:
        pass # Fails silently so it doesn't crash the flight software

def load_mission_state(filename, cycle_seconds):
    """Loads previous state if it exists and is from the current day cycle."""
    if os.path.exists(filename):
        try:
            with open(filename, 'r') as f:
                state = json.load(f)
            
            # Check if the recovered state belongs to the current day cycle
...
def start_experiment(sensors, filename):
    ...
    if recovered_state:
        day_start_time = recovered_state["day_start_time"]
        accumulated_lux_seconds = recovered_state["accumulated_lux_seconds"]
        daily_dose_delivered = recovered_state["daily_dose_delivered"]
    # --------------------------
    ...
    try:
        while True:
            ...
            fan_dc_temp = fan_dc_rh = 0.0 # Sub-trackers for the max() logic
            ...
            if not math.isnan(bme_t):
                # Calculate Fan Speed for Temp (Cools down)
                if bme_t > TEMP_THRESHOLD_HIGH:
                    fan_dc_temp = clamp_pwm(((bme_t - TEMP_THRESHOLD_HIGH) / P_BAND) * 100.0)
            ...
            if not math.isnan(bme_rh):
                # Calculate Fan Speed for Humidity (Dries out)
                if bme_rh > RH_THRESHOLD_HIGH:
                    fan_dc_rh = clamp_pwm(((bme_rh - RH_THRESHOLD_HIGH) / RH_P_BAND) * 100.0)

            # Fan runs at whichever demand is highest (Temperature vs Humidity)
            ...
            # ATOMIC STATE SAVE
            save_mission_state(state_filename, day_start_time, accumulated_lux_seconds, daily_dose_delivered)
