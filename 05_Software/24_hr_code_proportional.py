...
# Proportional Heating/Cooling Bands
P_BAND = 5.0              # Degrees below target to hit 100% heater power
RH_P_BAND = 10.0          # Humidity % over threshold to hit 100% fan power
        ...
        while True:
            ...
            if not math.isnan(therm_t):
                ...
                if therm_t < WATER_TEMP_LOW:
                    raw_water_power = clamp_pwm(((WATER_TEMP_LOW - therm_t) / P_BAND) * 100.0)
                    w_heat_dc = min(MAX_HEATER_POWER, raw_water_power) 
